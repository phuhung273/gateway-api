...
func (suite *ConformanceTestSuite) Setup(t *testing.T, tests []ConformanceTest) {
	...
	if supportsGateway {
		...
		tlsRouteSecret := kubernetes.MustCreateCASignedCertSecret(t, "gateway-conformance-infra", "tls-terminate-checks-certificate", []string{"tls.terminate.com"}, ca, caPrivKey)
		suite.Applier.MustApplyObjectsWithCleanup(t, suite.Client, suite.TimeoutConfig, []client.Object{tlsRouteSecret}, suite.Cleanup)
